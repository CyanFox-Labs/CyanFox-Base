...
class APIKeysTable extends DataTableComponent
{
    ...
    {
        return PersonalAccessToken::query()->where('tokenable_id', Auth::user()->id);
    }
    ...
    public function columns(): array
    {
        return [
            Column::make(__('messages.table.id'), 'id')
                ->sortable()
                ->searchable(),
            Column::make(__('account/profile.api_keys.table.name'), 'name')
                ->sortable()
                ->searchable(),
            Column::make(__('account/profile.api_keys.table.last_used'), 'last_used_at')
                ->sortable(),
