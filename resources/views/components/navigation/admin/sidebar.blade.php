    ...
    <nav class="bg-base-200 flex items-center justify-between p-3">
        <div class="dropdown dropdown-bottom dropdown-end ml-auto flex items-center">
            ...
            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                <li><a href="{{ route('home') }}"><i class='bx bxs-home'></i> {{ __('navigation.sidebar.home') }}</a></li>
                <li><a href="{{ route('profile') }}"><i class='bx bxs-user-circle'></i> {{ __('navigation.profile') }}
                    </a>
                </li>
                <div class="divider"></div>
                <li><a href="{{ route('logout') }}"><i class='bx bxs-log-out'></i> {{ __('navigation.logout') }}</a>
                </li>
            </ul>
        </div>
    </nav>
            ...
            <div class="flex flex-col items-center w-full mt-3 mb-3">
                <a class="flex items-center w-full h-12 px-3.5 mt-2 rounded hover:bg-gray-700 hover:text-gray-300 {{ request()->routeIs('admin') ? 'text-gray-300 bg-gray-700' : '' }}"
                   href="{{ route('admin') }}">
                    ...
                    <span class="ml-2 text-sm font-medium text-hidden">{{ __('navigation.sidebar.admin.dashboard') }}</span>
                </a>
                <a class="flex items-center w-full h-12 px-3.5 mt-2 rounded hover:bg-gray-700 hover:text-gray-300 {{ request()->routeIs('admin-user*') ? 'text-gray-300 bg-gray-700' : '' }}"
                   href="{{ route('admin-user-list') }}">
                    <i class="bx bxs-user-account"></i>
                    <span class="ml-2 text-sm font-medium text-hidden">{{ __('navigation.sidebar.admin.users') }}</span>
                </a>
                <a class="flex items-center w-full h-12 px-3.5 mt-2 rounded hover:bg-gray-700 hover:text-gray-300 {{ request()->routeIs('admin-role*') ? 'text-gray-300 bg-gray-700' : '' }}"
                   href="{{ route('admin-role-list') }}">
                    <i class="bx bxs-user-badge"></i>
                    <span class="ml-2 text-sm font-medium text-hidden">{{ __('navigation.sidebar.admin.roles') }}</span>
                </a>
            </div>
