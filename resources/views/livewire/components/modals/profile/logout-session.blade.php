<x-modal class="modal-bottom sm:modal-middle">
    ...
    <div class="text-center">
        <h2 class="text-2xl font-bold mb-4">{{ __('pages/account/modals.revoke_session.title') }}</h2>
        <p class="mb-3">{{ __('pages/account/modals.revoke_session.description') }}</p>
    </div>
    ...
    <div class="flex justify-center modal-action">
        <form method="dialog" class="flex gap-2" wire:submit="logoutSession">
            ...
            <x-button class="btn btn-danger"
                    type="submit" spinner="logoutSession">{{ __('pages/account/modals.revoke_session.buttons.revoke_session') }}
            </x-button>
        </form>
    </div>
</x-modal>
