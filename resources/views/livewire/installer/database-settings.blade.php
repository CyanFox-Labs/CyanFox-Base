<div>
    <x-form wire:submit="saveDatabaseSettings">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[repeat(auto-fit,minmax(0,1fr))] gap-4 mb-5">
            <x-input label="{{ __('installer.database.host') }}" class="input-bordered" wire:model="host" required/>
            <x-input label="{{ __('installer.database.port') }}" class="input-bordered" wire:model="port" required/>
            <x-input label="{{ __('installer.database.database') }}" class="input-bordered"
                     wire:model="database" required/>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-1">
            <x-input label="{{ __('installer.database.username') }}" class="input-bordered"
                     wire:model="username" required/>
            <x-input label="{{ __('installer.database.password') }}" class="input-bordered" type="password"
        ...
        <div class="mt-2 flex justify-between gap-3">
            <x-button class="btn btn-primary"
                ...
                {{ __('installer.database.buttons.test_connection') }}
            </x-button>
