...
@if ($component->isTailwind())
    <div
        ...
        {{ $attributes->merge($customAttributes['wrapper'])
            ->class(['shadow overflow-y-auto sm:rounded-lg' => $customAttributes['wrapper']['default'] ?? true])
            ->except('default') }}
    >
        <table
        ...
        >
            <thead wire:key="{{ $tableName }}-thead"
                {{ $attributes->merge($customAttributes['thead'])
                    ->class(['bg-base-200' => $customAttributes['thead']['default'] ?? true])
                    ->except('default') }}
            >
                <tr>
                    {{ $thead }}
            ...
            <tbody
                ...
                {{ $attributes->merge($customAttributes['tbody'])
                        ->class(['divide-y divide-gray-200 dark:divide-none' => $customAttributes['tbody']['default'] ?? true])
                        ->except('default') }}
            >
                {{ $slot }}
            </tbody>
