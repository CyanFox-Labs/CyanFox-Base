...
<head>
    ...
    <title>500 | {{ __('pages/errors.codes.500') }}</title>
    <link rel="stylesheet" href="{{ asset('css/error.css') }}">
</head>
<body>
<header>
    <h1 class="error-code">500</h1>
    <p class="error-description">{{ __('pages/errors.codes.500') }}</p>
</header>
<div class="status">
    <div class="card ok" id="client-status-card">
        <i class="icon">
    ...
    <div class="card error" id="server-status-card">
        <i class="icon">
            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000">
                <path d="M0 0h24v24H0V0z" fill="none"/>
                <path
        ...
        <p class="status-text">{{ __('pages/errors.codes.500') }}</p>
    </div>
</div>
<div class="reason">
    <div class="what-happened">
        <h2>{{ __('pages/errors.what_happened') }}</h2>
        <p class="description">{{ __('pages/errors.what_happened_descriptions.500') }}</p>
    </div>
    <div class="what-can-i-do">
        <h2>{{ __('pages/errors.what_can_i_do') }}</h2>
        <p class="description">{!! __('pages/errors.what_can_i_do_descriptions.try_again_later', ['here' => route('home')]) !!}</p>
    </div>
</div>
</body>
</html>
